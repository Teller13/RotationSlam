...
void load_data(const std::string& dataset_path)
{
  ...
  // 读相机内参
  {
    ...
    loaded_cam_calib.T_i_c.resize(2); //因为是double sphere相机 所以只有2个相机参数
  ...
  vec_T_w_i.resize(calib_corners.size() / 2); //因为同一个动作是两个相机在观察
  ...
  for (const auto& kv : calib_corners) {
    ...
    // 如果在calib_init_poses找得到该framecamID且camera_id是0
...
//用投影点误差法凭借ceres作优化 T_w_i 和 T_i_c 的位姿
void optimize() {
  ceres::Problem problem;
  //对于每一帧图片的标定板上每一个角点进行投影误差优化
  for (const auto& kv : calib_corners) {
    for(size_t i=0; i<kv.second.corners.size(); i++) {
      ...
      Eigen::Vector3d p_3d = aprilgrid.aprilgrid_corner_pos_3d[kv.second.corner_ids[i]];

      ReprojectionCostFunctor* c = new ReprojectionCostFunctor(p_2d, p_3d, cam_model);
      ceres::CostFunction* cost_function =
          new ceres::AutoDiffCostFunction<ReprojectionCostFunctor, 2, 7, 7, 8>(c);
